...
const Button: FC<IButtonProps> = ({
  ...
  switch (type) {
    case 'link':
      style = disabled ? 'border-solid border border-gray-200 bg-gray-200 cursor-not-allowed text-gray-800' : 'border-solid border border-gray-200 cursor-pointer text-primary-600 bg-white hover:shadow-sm hover:border-gray-300 transition-all duration-200'
      break
    case 'primary':
      style = (disabled || loading) ? 'bg-primary-600/75 cursor-not-allowed text-white' : 'bg-primary-600 hover:bg-primary-700 hover:shadow-md hover:-translate-y-[1px] cursor-pointer text-white transition-all duration-200'
      break
    default:
